...
def parse_args():
    ...
    # #parser.add_argument('--videoFile', type=str, required=True)->입력 받지 않음
    ...
    # args 추가
    args.cfg = '/home/sungeun/deep-high-resolution-net.pytorch-master/demo/inference-config.yaml'
...
def start_squat(vid, userNo):
    ...
    pose_model.load_state_dict(torch.load("/home/sungeun/deep-high-resolution-net.pytorch-master/models/pytorch/pose_coco/pose_hrnet_w32_256x192.pth"), strict=False)
    ...
    while vidcap.isOpened():
        ...
        if(vid==3):
            ...
            M = cv2.getRotationMatrix2D((cols/2,rows/2),270, rows/cols) #왼쪽으로 90도 회전시키기(오른쪽으로 90도는 270), 확대 1
        ...
        # 폰트
        fontpath = "/home/sungeun/deep-high-resolution-net.pytorch-master/demo/font/ITCKRIST.TTF" #폰트경로
        myfont = ImageFont.truetype(font = fontpath, size = 50) #폰트 type, size
...
def openImage():
    fullfilename = askopenfilename(initialdir="/home/sungeun/deep-high-resolution-net.pytorch-master/", 
    title="Select a file", filetypes=[("Video files", "*.avi *.mp4 *.f4v *.flv *.m4v *.mkv *.mov *.3gp *.mpeg *.mpg *.mts *.ts *.vob *.webm *.wmv *.gif"), ("All Files","*.*")]) #그 외 비디오타입 추가
...
def start(num, userNo):
    ...
    # root.geometry("250x100+700+10") #창 크기(너비x높이+x좌표+y좌표)
    # radVar=IntVar()
    ...
    # root.mainloop() #윈도우창 유지
    # vid = btn()
    root.withdraw()
    vid = num
    # 1 웹캠, 2 비디오(가로), 3 비디오(세로)
    if vid != None:
